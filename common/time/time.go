...
func ParseDuration(s string) (time.Duration, error) {
    ...
    // 避免处理错误的输入数据
    if len(s) > 100 {
        return 0, errors.New(fmt.Sprintf("time string s is too big:%s", s))
    }
    ...
    for i := 1; i < len(s); i++ {
        // -和+只允许出现在第一个字符
        if s[i] == '-' || s[i] == '+' {
            return 0, errors.New(fmt.Sprintf("invalid duration time s:%s", s))
        }
    }
    ...
    // TODO yy 处理数值溢出的情况
    daysPart := time.Hour * 24 * time.Duration(days)
    if len(right) == 0 {
        return daysPart, nil
    }
