...
This file provides project-level guidance for AI coding agents working with this repository.
...
- `cmd/demo/`: HTTP visualization server for demos and benchmarks
...
判断标准：
- 障碍发生在**我写的代码内部**（编译错误、测试失败）→ 自行修复
- 障碍涉及**目录结构、配置、.gitignore、git 操作失败**等设计决策 → 停下来报告，等待指示

git 操作失败是强信号——失败往往反映的是意图，不是偶发错误。
...
| 文件 | 内容 |
|------|------|
| `README.md` | 使用者视角：功能简介、快速上手示例、完整 API、注意事项 |
| `TEST.md` | 测试文件列表、分层运行命令（快速/全量/压测）、性能基线 |
| `DESIGN.md` | 核心选型理由、关键设计决策、踩过的坑 |

pre-commit hook 自动检查：若缺少任一文件，commit 被拒绝。纯常量包等例外情况用 `git commit --no-verify` 并在 message 中说明原因。
...
- Use `go run ./cmd/demo http` to start the visualization server at `http://localhost:8081`
- Visualizations are registered via `cmd/demo/registry.go`; each `api_*.go` file adds entries via `init()`
