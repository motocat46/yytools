...
## Engine 与 State 的职责分离
  ...
  - Engine 保存不可变规则（Config），可被多个玩家/对象共享，**goroutine-safe**。
  - State 保存单个玩家/对象的进度（当前位置、周期计划、已用配额、随机源），互相独立，**非 goroutine-safe**，不得跨 goroutine 共享。
  - 使用 Engine.NewState(r) 创建 State（r 为该玩家的随机源；无特殊层时可传 nil）。
