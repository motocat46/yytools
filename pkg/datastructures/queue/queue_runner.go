...
func queueOpEnqueue(queue *Queue[int], num int) {
	for i := 0; i < num; i++ {
		oldLength := queue.Len()
		queue.Enqueue(uniq)
		uniq++
...
func queueOpDequeue(queue *Queue[int], num int) {
	for i := 0; i < num; i++ {
		if !queue.Empty() {
...
func queueOpPeek(queue *Queue[int], num int) {
	for i := 0; i < num; i++ {
		if !queue.Empty() {
			oldLength := queue.Len()
			item := queue.Peek()
...
var queueHandlers = []func(queue *Queue[int], num int){
	queueOpEnqueue,
	queueOpDequeue,
	queueOpPeek,
}
