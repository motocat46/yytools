...
func priorityQueueOpPushItem(pq InterfacePriorityQueue[any], num int) interface{} {
	for i := 0; i < num; i++ {
		randomPriority := random.RandInt(1, 100000)
		one := &PriorityItem[any]{
			Data:     nil,
...
func priorityQueueOpPopItem(pq InterfacePriorityQueue[any], num int) interface{} {
	res := make([]*PriorityItem[any], 0, num)
	for i := 0; i < num; i++ {
		if pq.Length() > 0 {
			tmp := pq.PeekItem()
			item := pq.PopItem()
...
func priorityQueueOpPeekItem(pq InterfacePriorityQueue[any], num int) interface{} {
	oldLength := pq.Length()
	if oldLength > 0 {
		item := pq.PeekItem()
...
func priorityQueueOpUpdatePriority(pq InterfacePriorityQueue[any], num int) interface{} {
	for i := 0; i < num; i++ {
		oldLen := pq.Length()
		if oldLen > 0 {
			randomIndex := random.RandInt(0, pq.Length()-1)
			q := pq.(*PriorityQueue[any])
			randomPriority := random.RandInt(1, 100000)
...
var priorityQueueHandlers = []func(pq InterfacePriorityQueue[any], num int) interface{}{
...
func priorityQueueMustBeLegal(pq InterfacePriorityQueue[any]) {
	var deleted []*PriorityItem[any]
	items := priorityQueueOpPopItem(pq, pq.Length()).([]*PriorityItem[any])
	assert.Assert(pq.Length() == 0)
...
func PriorityQueueTest(num int) {
	...
	for i := 1; i <= num; i++ {
		fmt.Printf("第%d轮测试开始\n", i)
		for k, s := range scale {
			var pq InterfacePriorityQueue[any] = NewPriorityQueue[any]()
