...
func maxHeapOpPushItem(h InterfaceHeap[any], num int) interface{} {
	for i := 0; i < num; i++ {
		one := &Item[any]{
			Data:   nil,
			Weight: muniq,
...
func maxHeapOpPopItem(h InterfaceHeap[any], num int) interface{} {
...
func maxHeapOpPeekItem(h InterfaceHeap[any], num int) interface{} {
	for i := 0; i < num; i++ {
		oldLength := h.Length()
		if oldLength > 0 {
			item := h.PeekItem()
...
var maxHeapHandlers = []func(h InterfaceHeap[any], num int) interface{}{
	maxHeapOpPushItem,
	maxHeapOpPopItem,
	maxHeapOpPeekItem,
}

func maxHeapMustBeLegal(h InterfaceHeap[any], deleted []*Item[any]) {
	items := maxHeapOpPopItem(h, h.Length()).([]*Item[any])
	assert.Assert(h.Length() == 0)
...
func MaxHeapTest(num int) {
	...
	for i := 1; i <= num; i++ {
		fmt.Printf("第%d轮测试开始\n", i)
		for k, s := range scale {
			var h InterfaceHeap[any] = NewMaxHeap[any]()
			deleted := []*Item[any]{}
			muniq = 1000000
			mmax = muniq
