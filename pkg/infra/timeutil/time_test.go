...
func TestOverflowDetection(t *testing.T) {
	t.Run("乘法溢出检测", func(t *testing.T) {
		// 测试正常情况
		result, overflow := mul(2, 3)
		if overflow || result != 6 {
			t.Errorf("mul(2, 3) = (%d, %t), 期望 (6, false)", result, overflow)
		}

		// 测试溢出情况
		result, overflow = mul(1<<62, 2)
		if !overflow {
			t.Errorf("mul(1<<62, 2) 应该溢出")
		}
	})

	t.Run("加法溢出检测", func(t *testing.T) {
		// 测试正常情况
		result, overflow := add(1, 2)
		if overflow || result != 3 {
			t.Errorf("add(1, 2) = (%d, %t), 期望 (3, false)", result, overflow)
		}

		// 测试溢出情况
		result, overflow = add(1<<63-1, 1)
		if !overflow {
			t.Errorf("add(1<<63-1, 1) 应该溢出")
		}
	})

	t.Run("减法溢出检测", func(t *testing.T) {
		// 测试正常情况
		result, overflow := sub(3, 2)
		if overflow || result != 1 {
			t.Errorf("sub(3, 2) = (%d, %t), 期望 (1, false)", result, overflow)
		}

		// 测试溢出情况
		result, overflow = sub(-1<<63, 1)
		if !overflow {
			t.Errorf("sub(-1<<63, 1) 应该溢出")
		}
	})
}
