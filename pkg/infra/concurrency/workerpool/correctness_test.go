...
// WorkerPool 的每条命题对 Mutex 和 RWMutex 两种实现均运行一遍。
...
func TestCorrectness_ExactlyOnce_ConcurrentSubmitClose(t *testing.T) {
	const (
		submitters = 20
		perWorker  = 5_000
	)
...
//
// 验证锁保护下，Submit 与 Close 并发不触发任何 panic。
...
// Pipeline 内部使用 WorkerPool，此处不重复对锁实现分组，直接用默认实现。
