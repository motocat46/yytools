...
func opInSliceFunc[T base.Ordered](s []T, compare func(a T, b T) bool) (int, T) {
	assert.Assert(len(s) > 0, "len(s):", len(s))
	index := 0
	val := s[0]
	for i := 1; i < len(s); i++ {
		v := s[i]
...
func MinInSlice[T base.Ordered](s []T) (int, T) {
	// slices.Min()不返回最小值的下标，这里扩展返回最小值下标
	return opInSliceFunc(s, func(a T, b T) bool {
		return a < b
	})
}
