...
func heapOpPushItem(h heap.InterfaceHeap[any], num int) interface{} {
	for i := 0; i < num; i++ {
		one := &heap.Item[any]{Data: nil, Weight: uniq}
		h.PushItem(one)
		uniq++
...
func heapOpPopItem(h heap.InterfaceHeap[any], num int) interface{} {
...
func heapOpPeekItem(h heap.InterfaceHeap[any], num int) interface{} {
	for i := 0; i < num; i++ {
		oldLength := h.Length()
		if oldLength > 0 {
			item := h.PeekItem()
...
var heapHandlers = []func(h heap.InterfaceHeap[any], num int) interface{}{
	heapOpPushItem,
	heapOpPopItem,
	heapOpPeekItem,
}

func heapMustBeLegal(h heap.InterfaceHeap[any], deleted []*heap.Item[any]) {
	items := heapOpPopItem(h, h.Length()).([]*heap.Item[any])
	assert.Assert(h.Length() == 0)
...
func HeapTest(num int) {
	...
	for i := 1; i <= num; i++ {
		fmt.Printf("第%d轮测试开始\n", i)
		for k, s := range scale {
			var h heap.InterfaceHeap[any] = heap.NewHeap[any]()
